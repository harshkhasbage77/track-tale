...
//     // Define scale factor at the top (e.g., 10 pixels per second)
...
//         const handleResize = () => {
//             if (ref.current) {
//                 // Ensure consistent space per second (e.g., 10px per second)
//                 const newWidth = (max / 1000) * scaleFactor;
...
//                     ctx.beginPath();
//                     for (let i = 0; i <= max; i += marking.interval) {
//                         const xPos = (i / 1000) * scaleFactor;
//                         ctx.moveTo(xPos, 0);
//                         ctx.lineTo(xPos, marking.size);
//                     }
//                     ctx.stroke();
//                 });
//             }
...
export const ScaleRangeInput: React.FC<ScaleRangeInputProps> = (props) => {
    ...
    useEffect(() => {
        // Update canvas size based on max and scaleFactor
        const handleResize = () => {
            if (ref.current) {
                const newWidth = (max / 1000) * scaleFactor;
    ...
    }, [max, props.height]);  // added max and props.height as dependencies
    ...
    useEffect(() => {
        if (ref.current) {
            ...
            if (ctx) {
                ...
                // Draw markings with consistent spacing
                props.markings.forEach((marking) => {
                    ctx.strokeStyle = marking.color;
                    ctx.lineWidth = marking.width;
                    ctx.beginPath();
